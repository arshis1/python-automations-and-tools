'''
# Codeforces Problem 681C - Heap Operations
C. Heap Operations
...
def getMin(heap, x):
    log_op = []
    while heap[0] < x:
        log_op.append(removeMin(heap))
    log1 =   "getMin " + str(x)
    if len(heap) ==0 or heap[0] > x:
        log_op.append(insert(heap, x))
    log_op.append(log1)
    return log_op
...
for _ in range(n_log):
    ...
    if command[0] == "insert":
        command_load.append(insert(h, int(command[1])))
    elif command[0] == "getMin":
         getmin_op = getMin(h, int(command[1]))
         for cmd in getmin_op:
             if cmd is not None:
                 command_load.append(cmd)
    else:
        if len(h) ==0:
            command_load.append(insert(h, 0))    
        command_load.append(removeMin(h))

print(len(command_load))
for c in command_load:
    print(c)
